...
class PromptStore:
    ...
    async def save_prompt(self, prompt):
        ...
        """
        try:
            insert_prompt = {'_key': 'test', 'user': prompt.user, 'content': prompt.prompt_text }
           
            inserted_prompt_metadata = self.collection.insert(insert_prompt)
            # prompt_id = str(insert_prompt_metadata["_key "])
            if  inserted_prompt_metadata['_key'] == 'test':
    ...
    def delete_prompt(self, prompt_id) -> bool:
        ...
        try:
            # _id = ObjectId(prompt_id)
            # doc=await self.get_user_prompt_by_id(prompt_id)
            result = self.collection.delete(prompt_id)
